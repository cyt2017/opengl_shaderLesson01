...
void QGLESWIDGET::render()
{
    ...
    {
       vect[i].u   *=  0.25f;//改变起始位置
       vect[i].v   *=  0.25f;

       vect[i].u   +=  0.25 * col;//偏移一个单位距离
       vect[i].v   +=  0.25 * row;
    }

    _uvOffset = CELL::float2(0.25f,0.25f);
#else
...
#if !USE_SHADER
        glUniform2f(_shader._offset,_uvOffset.x,_uvOffset.y);
#else
        glUniform3f(_shader._offset,4.0,4.0,frame);
#endif
